## MVCI Notes
Moved all code into `salesforce_blip/`.
...
### Uploading version to artifactory

```bash
# doesn't have to be very precise, just increase the number from the previous version
rm -rf dist
poetry version <version>
poetry build
artifactory-twine upload --repository-url https://artifactory.secureserver.net/artifactory/api/pypi/pypi-creative-intelligence-test-local --username <username> --password <password> dist/*
```
